...
std::string MSWindowsClipboardBitmapConverter::toIClipboard(HANDLE data) const
{
  ...
  if (w <= 0 || h == 0 || h == std::numeric_limits<LONG>::min()) {
    LOG_WARN("invalid clipboard bitmap dimensions: %dx%d", w, h);
    GlobalUnlock(data);
    return std::string();
  }
  const LONG rowCount = h < 0 ? -h : h;
  const size_t width = static_cast<size_t>(w);
  const size_t height = static_cast<size_t>(rowCount);
  if (width > std::numeric_limits<size_t>::max() / 4 / height) {
    LOG_WARN("clipboard bitmap dimensions are too large: %dx%d", w, h);
    GlobalUnlock(data);
    return std::string();
  }
  const size_t imageSize = 4 * width * height;
  if (imageSize > std::string().max_size() - sizeof(BITMAPINFOHEADER)) {
    LOG_WARN("clipboard bitmap data is too large: %zu bytes", imageSize);
    GlobalUnlock(data);
    return std::string();
  }
  ...
  if (dc == nullptr) {
    LOG_WARN("failed to get display context for clipboard image");
    GlobalUnlock(data);
    return std::string();
  }
  ...
  if (dstDC == nullptr) {
    LOG_WARN("failed to create display context for clipboard image");
    DeleteObject(dst);
    ReleaseDC(nullptr, dc);
    GlobalUnlock(data);
    return std::string();
  ...
  SetDIBitsToDevice(dstDC, 0, 0, w, rowCount, 0, 0, 0, rowCount, srcBits, bitmap, DIB_RGB_COLORS);
